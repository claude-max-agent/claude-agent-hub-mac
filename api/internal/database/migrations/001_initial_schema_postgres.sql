-- Migration: 001_initial_schema_postgres
-- Created: 2026-02-01
-- Description: Initial database schema for Raspy Agent (PostgreSQL)
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    CONSTRAINT tasks_valid_type CHECK (type IN ('development', 'research', 'notification', 'feature', 'bugfix')),
    CONSTRAINT tasks_valid_priority CHECK (priority IN ('high', 'medium', 'low')),
    CONSTRAINT tasks_valid_status CHECK (status IN ('pending', 'assigned', 'in_progress', 'completed', 'cancelled', 'blocked', 'archived', 'expired'))
);
...
CREATE TABLE IF NOT EXISTS agent_statuses (
    ...
    CONSTRAINT agent_statuses_valid_status CHECK (status IN ('available', 'busy', 'stopped', 'idle'))
);
...
CREATE TABLE IF NOT EXISTS requests (
    ...
    CONSTRAINT requests_valid_status CHECK (status IN ('pending', 'approved', 'rejected', 'executed'))
);
...
-- =============================================================================
-- 7. pending_questions (User Questions)
-- =============================================================================
CREATE TABLE IF NOT EXISTS pending_questions (
    id VARCHAR(64) PRIMARY KEY,
    short_id VARCHAR(8) UNIQUE,
    agent_id VARCHAR(32) NOT NULL,
    question_type VARCHAR(32) NOT NULL DEFAULT 'question',
    ...
    CONSTRAINT pending_questions_valid_type CHECK (question_type IN ('question', 'confirmation', 'choice')),
    CONSTRAINT pending_questions_valid_status CHECK (status IN ('pending', 'answered', 'cancelled'))
);

CREATE INDEX IF NOT EXISTS idx_pending_questions_status ON pending_questions(status);
CREATE INDEX IF NOT EXISTS idx_pending_questions_agent_id ON pending_questions(agent_id);
CREATE INDEX IF NOT EXISTS idx_pending_questions_short_id ON pending_questions(short_id);

-- =============================================================================
-- 8. Helper function: Update updated_at timestamp
-- =============================================================================
...
-- =============================================================================
-- 9. Add parent_task_id for task hierarchy (subtasks)
-- =============================================================================
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE tasks ADD COLUMN parent_task_id VARCHAR(64) REFERENCES tasks(id) ON DELETE CASCADE;
...
INSERT INTO schema_migrations (version) VALUES ('001_initial_schema_postgres')
